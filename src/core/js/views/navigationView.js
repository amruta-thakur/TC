    ...
    var NavigationView = Backbone.View.extend({
        ...
        hideNavigationButton: function(model) {
            if (model.get('_type') === "course") {
                //added customized code to hide page title on navigation bar f0r course 
                $('.navigation').addClass('display-none');
                $('#wrapper').addClass('removePadding');
                 /***********************Ended******************/
                $('.navigation-back-button, .navigation-home-button').addClass('display-none');
               
                this.$('.navigation-label').html('');
            } else {
                //added customized code to show page title on navigation bar
                 this.$('.navigation-label').html(model.get('title'));
                 $('.navigation').removeClass('display-none');
                 $('#wrapper').removeClass('removePadding');
                 /***********************Ended******************/
